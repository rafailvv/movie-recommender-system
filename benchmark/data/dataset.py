...
class MovieDataset(Dataset):
    def __init__(self, dataframe):
        ...
        self.genders = torch.tensor(
            dataframe[["gender_F", "gender_M"]].values, dtype=torch.float32
        )
        self.genre_features = torch.tensor(
            dataframe[list(all_genres)].values, dtype=torch.float32
        )
